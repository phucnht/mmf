...
export default function CardCharacter({ imgSrc, className, content, children }: CardCharacterProps) {
  ...
  return (
    <Box className="relative h-full w-full flex items-start justify-center bg-[url('/assets/bg/bg-character.svg')] bg-[length:46.3rem_100%] bg-no-repeat bg-center">
      <Image
        alt={content}
        src={imgSrc}
        layout="fill"
        objectFit="cover"
        className="character-frame-clip scale-[96%] translate-x-2 translate-y-6"
      />
      <SvgCharacterFrame />
      <span className='absolute bottom-16 left-16 bg-[url("/assets/inventory/characters/cf-flower-left.png")] w-[5.4rem] h-[4.6rem] bg-auto bg-no-repeat bg-center' />
      <span className='absolute bottom-16 right-[5.5rem] bg-[url("/assets/inventory/characters/cf-flower-right.png")] w-[4.8rem] h-[4.1rem] bg-auto bg-no-repeat bg-center' />
      <Stack className="absolute bottom-1">
