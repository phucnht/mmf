...
const CardItem = ({ item, onListing }: { item: ItemType; onListing?: boolean }) => {
  return (
    <Paper elevation={0} className='p-5 rounded-[8px] hover:shadow-card'>
      ...
      <CardMedia
        image={item.external.backgroundUrl}
        className='flex justify-center items-center h-[240px] relative p-8 mt-3 border'
      >
        <div className='absolute top-3 left-3'>
          <ChipItemRarity rarity={item.external.rarity} />
        </div>
        <img src={item.external.iconUrl} className='max-h-full border' />
      </CardMedia>
    </Paper>
  );
};
